...
def build_mlp(input_placeholder, output_size, scope, n_layers, size,
    ...
    """
    mlp_output = input_placeholder
    with tf.variable_scope(scope):
        for i, layer in enumerate(n_layers):
            mlp_input = mlp_output
            if i == n_layers - 1:
                # output layer
                mlp_output = tf.layers.dense(mlp_input, output_size)
                if output_activation is not None:
                    mlp_output = output_activation(mlp_output)
            else:
                # feed-forward layer
                mlp_output = tf.layers.dense(mlp_input, size)
                mlp_output = activation(mlp_output)
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            sy_mean = build_mlp(sy_ob_no, self.ac_dim,
                                scope="policy_forward_pass",
                                n_layers=self.n_layers, size=self.size)
            sy_logstd = tf.get_variable("log_std_var",
                                        [self.ac_dim],
                                        dtype=tf.float32)
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            sy_sampled_ac = tf.arg_max(sy_logits_na, dimension=1)
        else:
            sy_mean, sy_logstd = policy_parameters
            unit_samples = tf.random_normal(tf.shape(sy_mean),
                                            name="repara_unit_gaussian")

            sy_sampled_ac = unit_samples * sy_logstd + sy_mean
